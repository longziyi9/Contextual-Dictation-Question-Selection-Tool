...
class App:
            ...
            if var.get():
                ...
                for _, row in chosen.iterrows():
                    selected_items.append({
                        ...
                        'answer_c': str(row.iloc[4]) if pd.notna(row.iloc[4]) else ""   # 新增C列答案
                    })
        if not has_checked:
    ...
    def _build_question_and_answer(self, items):
        questions = []
        answers_parts = []
        ...
        for idx, item in enumerate(items, start=1):
            ...
            for ch in stem:
                if ch == 'A':
                    q_chars.append('_________________________')
                elif ch == 'B':
                    q_chars.append('_________________________')
                elif ch == 'C':
                    continue          # 直接删除
            ...
            for pos, ch in positions:
                if pos > last_end:
                    parts.append((stem[last_end:pos], False))
                answer_text = placeholder_map[ch]
                if answer_text:                     # 只有非空才添加带下划线的片段
                    parts.append((answer_text, True))
                last_end = pos + 1
